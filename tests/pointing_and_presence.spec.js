...
test.describe('Pointing System, Instant Frame Loading & Lobby Presence', () => {
  test('Casual chat words do NOT award points, only real full-title guesses score points', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      // Casual chat phrases that used to trigger false matches
      const casualPhrases = [
        'idiot',
        'idiots',
        'the movie',
    ...
    // Real guess MUST match
    expect(result.realMatches[0]).toBe(true);
    expect(result.realMatches[1]).toBe(true);
    expect(result.realMatches[2]).toBe(true);
  });
