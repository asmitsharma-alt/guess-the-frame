/**
 * Authoritative Game Catalog for ScoopCast – Guess the Frame
 * Cloudflare Durable Object Server Catalog
 */
...
export interface CatalogItem {
  ...
  sectionId?: number;
  sectionName: string;
  category: 'frames' | 'eyes' | 'dialogue' | 'tiebreaker';
  type: 'image' | 'dialogue';
  content: string;
  answer: string;
  year?: string;
...
export const DEFAULT_FRAMES: CatalogItem[] = [
...
export const DEFAULT_EYES: CatalogItem[] = [
...
export const DEFAULT_DIALOGUES: CatalogItem[] = [
...
export const DEFAULT_TIE_BREAKERS: CatalogItem[] = [
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Anatomy of a Fall (2023).webp", answer: "ANATOMY OF A FALL", year: "2023" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Eyes Wide Shut (1999).webp", answer: "EYES WIDE SHUT", year: "1999" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Ghilli (2004).webp", answer: "GHILLI", year: "2004" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/La Haine(1995).webp", answer: "LA HAINE", year: "1995" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Mad Max 2.jpg.webp", answer: "MAD MAX 2", year: "1981" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Moonrise Kingdom (2012).webp", answer: "MOONRISE KINGDOM", year: "2012" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/The Batman (2022).webp", answer: "THE BATMAN", year: "2022" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/The Holdovers(2023).webp", answer: "THE HOLDOVERS", year: "2023" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/The Life of Chuck(2024).webp", answer: "THE LIFE OF CHUCK", year: "2024" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/The Lighthouse (2019).webp", answer: "THE LIGHTHOUSE", year: "2019" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/The Wolf of Wall Street (2013).webp", answer: "THE WOLF OF WALL STREET", year: "2013" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/They Call Him OG (2025).webp", answer: "THEY CALL HIM OG", year: "2025" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Top Gun Maverick (2022).webp", answer: "TOP GUN MAVERICK", year: "2022" },
  { sectionId: 4, sectionName: "Tie Breaker", category: "tiebreaker", type: "image", content: "tie breaker/Under the Silver Lake (2018).webp", answer: "UNDER THE SILVER LAKE", year: "2018" }
];

export const CATALOG_BY_CATEGORY: Record<string, CatalogItem[]> = {
...
/**
 * Generates an authoritative match playlist on the server
 */
export function createMatchPlaylist(
  categories: string[] = ['frames', 'eyes', 'dialogues'],
  totalRounds: number = 20
): CatalogItem[] {
  let pool: CatalogItem[] = [];

  const safeCategories = categories && categories.length > 0 ? categories : ['frames', 'eyes', 'dialogues'];

  for (const cat of safeCategories) {
    const norm = cat.toLowerCase();
    const items = CATALOG_BY_CATEGORY[norm] || CATALOG_BY_CATEGORY[norm.replace(/s$/, '')] || [];
    pool = pool.concat(items);
  }

  // Fallback if empty
  if (pool.length === 0) {
    pool = [...DEFAULT_FRAMES, ...DEFAULT_EYES, ...DEFAULT_DIALOGUES];
  }

  // Shuffle pool with Fisher-Yates
  const shuffled = shuffleArray(pool);

  // Return requested rounds (or up to pool length)
  const count = Math.min(totalRounds, shuffled.length);
...
/**
 * Sanitizes a catalog frame for active round broadcast by removing answer fields
 */
export function sanitizeFrameForActiveRound(frame: CatalogItem | null): Omit<CatalogItem, 'answer' | 'revealContent'> | null {
  if (!frame) return null;
  const { answer, revealContent, ...safe } = frame;
  return safe;
}
