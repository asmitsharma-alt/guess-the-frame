...
function generateAppwriteEngine() {
  return `    /* ══════════════════════════════════════════════════════════════════
       PRODUCTION APPWRITE MULTIPLAYER ENGINE (EXCLUSIVE BACKEND)
       ══════════════════════════════════════════════════════════════════ */
    ...
    const MultiplayerEngine = {
      ...
      setupLifecycleListeners() {
        document.addEventListener('visibilitychange', async () => {
          ...
          if (document.hidden) {
            ...
            console.log('[Lifecycle] Tab woke up. Refreshing state...');
            try {
              await databases.updateDocument(APPWRITE_CONFIG.databaseId, 'players', playerDocId, {
                status: 'connected',
      ...
      async confirmJoinRoom() {
        ...
        try {
          const roomsQuery = await databases.listDocuments(APPWRITE_CONFIG.databaseId, 'rooms', [
          ...
          console.error('[Appwrite Join Room Error]', err);
      ...
      subscribeRealtime() {
        ...
        console.log('[Realtime] Subscribing to Appwrite channels for room:', this.roomId);
      ...
      handleRealtimeEvent(response) {
        const eventName = response.events[0] || '';
      ...
      handleGameStateUpdate(state) {
        if (!state) return;
        if (state.version && state.version < this.gameStateVersion) return;
        this.gameStateVersion = state.version || this.gameStateVersion;
      ...
      async syncActiveRoomState() {
        ...
        try {
          const pList = await databases.listDocuments(APPWRITE_CONFIG.databaseId, 'players', [
          ...
          const state = await databases.getDocument(APPWRITE_CONFIG.databaseId, 'game_state', this.roomId);
          this.handleGameStateUpdate(state);

          this.renderLobbyUI();
          UI.renderLB();
        } catch (err) {
          console.warn('[Sync Active Room State]', err.message);
        }
      },
      ...
      startHeartbeat() {
        ...
        this.presenceWatchdogTimer = setInterval(() => {
          ...
          GS.players.forEach(p => {
            ...
            if (diff > 45000) newStatus = 'disconnected';
            else if (diff > 30000) newStatus = 'reconnecting';
            else if (diff > 10000) newStatus = 'away';
          ...
          try {
            ...
            if (result.success) {
              ...
              if (result.isCorrect) {
                ...
                if (p) p.score = (p.score || 0) + result.points;
                UI.renderLB();
                ...
                if (this.isHost) {
                  await this.recordRoundWinner(result.winRecord, result.points);
                }
              } else {
            ...
            console.warn('[Appwrite Function Check Fallback]', e.message);
          }
        }

        if (!validated && this.isHost) {
      ...
      async confirmRejoinRoom() {
        ...
        if (databases) {
          try {
            const roomDoc = await databases.getDocument(APPWRITE_CONFIG.databaseId, 'rooms', this.roomId);
            if (roomDoc.status === 'playing') {
              this.executeGameStart();
            } else {
              UI.showScreen('playerLobbyScreen');
...
function updateHtmlFile(filePath) {
  ...
  // 2. Locate MultiplayerEngine boundaries
  ...
  if (startIdx === -1 || endIdx === -1) {
    console.error('Could not find exact MultiplayerEngine markers in', filePath);
    return;
  }

  const newEngine = generateAppwriteEngine().trim() + '\n\n    ';
  content = content.slice(0, startIdx) + newEngine + content.slice(endIdx);

  // 3. Optimize FrameDisplay.preloadAll to eliminate 130MB PNG flood and use Round 1 only
  const preloadAllMarker = 'async preloadAll(onProgress) {';
  const preloadStart = content.indexOf(preloadAllMarker);
  if (preloadStart !== -1) {
    const nextMethodMarker = '/* Preload next few frames */';
    const preloadEnd = content.indexOf(nextMethodMarker, preloadStart);
    if (preloadEnd !== -1) {
      const optimizedPreload = `async preloadAll(onProgress) {
        const round1 = (GS.sections && GS.sections[0] && GS.sections[0].frames[0]) ? GS.sections[0].frames[0] : null;
        const priorityImages = [];

        // Preload Round 1 media only (instant lobby entry)
        if (round1) {
          if (round1.content) priorityImages.push(round1.content);
          if (round1.revealContent) priorityImages.push(round1.revealContent);
        }

        // Current player avatar SVG
        GS.players.forEach(p => {
      ...
      `;
      content = content.slice(0, preloadStart) + optimizedPreload + content.slice(preloadEnd);
    }
  }

  // 4. Update preloadImage to prefer .webp
  content = content.replace(
    /preloadImage\(src\)\s*\{\s*return new Promise\(\(resolve,\s*reject\)\s*=>\s*\{/g,
    `preloadImage(src) {
        ...
        return new Promise((resolve) => {
          const img = new Image();
          img.onload = () => { GS.imageCache.set(webpSrc, img); resolve(img); };
          img.onerror = () => {
            const fallback = new Image();
            fallback.onload = () => { GS.imageCache.set(src, fallback); resolve(fallback); };
      ...
      _unusedPreloadImage(src) { return new Promise((resolve, reject) => {`
  );
...
console.log('Appwrite migration applied with all controllers preserved!');
